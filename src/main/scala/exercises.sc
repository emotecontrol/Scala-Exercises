...
def split(n: Int, list: List[Any]): (List[Any], List[Any]) = {
  def splitHelper(count: Int, l: List[Any], agg: (List[Any], List[Any])): (List[Any], List[Any]) = count match {
...
def insertAt(add: Any, n: Int, list: List[Any]): List[Any] = {
  val splitList = split(n, list)
...
def randomSelect(n: Int, list: List[Any]) = {
  val r = new scala.util.Random()
  def randomHelper(count: Int, l: List[Any], agg: List[Any]): List[Any] = {
    if (count >= n) agg
    else {
...
def lotto(n: Int, m: Int):List[Int] = {
  def generateList(limit: Int, agg:List[Int]): List[Int] = {
    if (agg.length == limit) agg
    else generateList(limit, agg :+ (last(agg).asInstanceOf[Int] + 1))
  }
  randomSelect(n, generateList(m, List(1))).asInstanceOf[List[Int]]
}
lotto(6, 49)

// P25 Generate a random permutation of the elements of the list
def randomPermute(list: List[Any]) = {
  randomSelect(length(list), list)
}
randomPermute(List('a, 'b, 'c, 'd, 'e, 'f))

// P26 Generate the combinations of K distinct objects chosen from the N elements of a list.
def combinations(K: Int, list: List[Any]) = {
  def comb(l:List[Any], agg: List[List[Any]]):List[List[Any]] = {
    if (agg.head.length == K) agg
    else if (l == List()) List()
    else comb(l.tail, List(agg.head :+ l.head)) ++ comb(l.tail, agg)
  }
  comb(list, List(Nil))
}
combinations(3, List('a, 'b, 'c, 'd, 'e))
